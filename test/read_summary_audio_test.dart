...
void main() {
  testWidgets(
    'confirms the selected audio and replaces the captured reminder',
    (tester) async {
      ...
      final confirmButton = find.byKey(
        const Key('confirmar-read-summary-button'),
      );
  ...
  testWidgets('correct returns to audio capture without confirming the batch', (
    tester,
  ) async {
    await tester.binding.setSurfaceSize(const Size(360, 800));
    addTearDown(() => tester.binding.setSurfaceSize(null));
  ...
  testWidgets('close discards the audio batch without updating home', (
    tester,
  ) async {
    await tester.binding.setSurfaceSize(const Size(360, 800));
    addTearDown(() => tester.binding.setSurfaceSize(null));
...
GoRouter _buildRouter() => GoRouter(
  initialLocation: '${ReadSummaryPage.routePath}?source=audio',
  routes: <RouteBase>[
    GoRoute(
